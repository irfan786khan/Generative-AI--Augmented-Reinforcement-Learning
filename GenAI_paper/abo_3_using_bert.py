...
if __name__ == "__main__":
    ...
    plt.subplot(1, 3, 3)
    plt.hist(test_stabilities, bins=20, alpha=0.7)
    plt.axvline(x=np.mean(test_stabilities), color='r', linestyle='--', label=f'Mean: {np.mean(test_stabilities):.3f}')
    plt.axvline(x=np.min(test_stabilities), color='g', linestyle='--', label=f'Best: {np.min(test_stabilities):.3f}')
